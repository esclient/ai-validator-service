...
def load_parquet_shards() -> pd.DataFrame:
    ...
    for prefix, lang_code in PARQUET_LANGS.items():
        ...
        try:
            ...
            frames.append(df)
            log.info(f"{filename}: {len(df):,} rows (lang={lang_code})")
        except Exception as exc:
            log.error(f" failed in loading {filename}: {exc}")
...
def load_inappropriate_messages() -> pd.DataFrame:
    ...
    log.debug(f"CSV columns: {list(raw.columns)}")
    ...
    try:
        ...
        log.info("No label column in CSV - treating all rows as toxic (1)")
    ...
    lang_col = next(
        (
            ...
            if c.lower() in ("lang", "language", "locale")
        ),
        None,
    )
    ...
    return base_frame(
        text, label, lang, "inappropriate_messages", "inappropriate"
    )
...
def load_labled_csv() -> pd.DataFrame:
    ...
    log.debug(f"CSV columns: {list(raw.columns)}")
    text = _text_col(raw)
    label = _to_binary_label(
        raw["abusive"].map({"True": 1, "False": 0, True: 1, False: 0})
    )
...
def load_russian_jsonl() -> pd.DataFrame:
    ...
    log.debug(f"JSONL columns: {list(raw.columns)}")
    text = _text_col(raw)
    label = _label_from_parquet(raw)
...
def load_russian_tsv2() -> pd.DataFrame:
    ...
    log.debug(f"TSV columns: {list(raw.columns)}")

    toxic = base_frame(
        raw["ru_toxic_comment"],
...
def load_russian_dataset_3() -> pd.DataFrame:
    ...
    return base_frame(
        raw["text"],
        _to_binary_label(raw["label"]),
        "ru",
        "russian_dataset_3_txt",
    )
...
def run() -> None:
    ...
    for loader in [load_civil_comments]:
        try:
            df = loader()
            log.info(f"{loader.__name__}: {len(df):,} rows")
            frames.append(df)
        except Exception as exc:
            log.error(f"{loader.__name__} FAILED: {exc}")

    log.info("Loading parquet shards")
    try:
        df = load_parquet_shards()
        log.info(f"Parquet total: {len(df):,} rows")
        frames.append(df)
    except Exception as exc:
        log.error(f"Parquet shards FAILED: {exc}")

    log.info("Loading local files")
    local_loaders = [
    ...
    log.info(f"Raw combined: {len(df):,} rows")
    ...
    log.info(f"After dedup + filter: {len(df):,} rows")

    log.info("Label distribution")
    log.debug(
        df["label"].value_counts().rename({0: "clean", 1: "toxic"}).to_string()
    )
    log.info("Language distribution")
    log.debug(df["lang"].value_counts().to_string())
    ...
    for split, name in [(train, "train"), (val, "val"), (test, "test")]:
        ...
        log.info(f"Saved {name}: {len(split):,} rows -> {path}")
