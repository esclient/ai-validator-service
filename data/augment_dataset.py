...
def build_toxic_vocabulary(
    df: pd.DataFrame, top_n: int = TOP_N_TOXIC_WORDS
) -> set[str]:
    """
    ...
    log.info(
        f"[vocab] Bootstrapped {len(toxic_vocab)} toxic indicator words via TF-IDF"
    )
    return toxic_vocab
...
def aug_crosslang_concat(
    ...
    **_,
    ...
    other_langs = [
        lang_code
        for lang_code in lang_pool
        if lang_code != lang and lang_pool[lang_code]
    ]
    if not other_langs:
        return None
...
def _target_counts(
    ...
    max_allowed = int(
        total_real * MAX_SYNTHETIC_RATIO / (1 - MAX_SYNTHETIC_RATIO)
    )
...
def build_augmented_dataset(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for rec in random.choices(toxic_df.to_dict("records"), k=n_toxic):
        ...
        add_row(
            text, 1, rec["lang"], "synthetic_partial_concat", "concatenation"
        )
    ...
    for t_rec, c_rec in zip(toxic_recs, clean_recs, strict=False):
        text, label, category = _build_context_mix_row(
    ...
    if synth_df.empty:
        log.error("[augment] no synthetic rows generated.")
        return synth_df

    synth_df["label"] = synth_df["label"].astype("int8")
    log.info(f"[augment] Synthetic rows generated: {len(synth_df):,}")
    log.debug(
        synth_df.groupby(["source", "category", "label"]).size().to_string()
    )
    return synth_df
...
def augment_and_combine(real_df: pd.DataFrame) -> pd.DataFrame:
    ...
    log.info(f"[augment] Final combined dataset: {len(combined):,} rows")
    log.info(f"[augment] Real: {len(real_df):,}")
    log.info(f"[augment] Synthetic: {len(combined) - len(real_df):,}")
    log.info(
        f"  Synthetic ratio: {(len(combined) - len(real_df)) / len(combined):.1%}"
    )
    return combined
