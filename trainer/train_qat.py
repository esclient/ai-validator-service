import os, numpy as np, pandas as pd, torch, shutil, time
...
class SaveBestCallback(TrainerCallback):
    ...
    def on_evaluate(self, args, state, control, metrics=None, **kwargs):
        ...
        if combined > self.best_score:
            ...
            print(f"New best {combined:.4f} saved → {self.save_path}")
...
def stratified_sample(df, n, label_col="label", seed=42):
    return (
        df.groupby(label_col, group_keys=False)
          .apply(lambda x: x.sample(min(len(x), int(n * len(x) / len(df))), random_state=seed), include_groups=True)
...
def language_balanced_sample(df, en_cap=300_000, ru_cap=300_000, other_per_lang=8_000, seed=42):
    ...
    for lang, group in df.groupby("lang"):
        cap   = {"en": en_cap, "ru": ru_cap, "mixed": 20_000}.get(lang, other_per_lang)
        cap_n = min(len(group), cap)
        parts.append(
            group.groupby("label", group_keys=False)
                 .apply(lambda x: x.sample(min(len(x), int(cap_n * len(x) / len(group))), random_state=seed), include_groups=True)
                 .sample(frac=1, random_state=seed)
                 .reset_index(drop=True)
        )
...
print(f"Weights — clean: {class_weights[0]:.3f}, toxic: {class_weights[1]:.3f}")
