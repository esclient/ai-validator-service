...
class SaveBestToDriveCallback(TrainerCallback):
    ...
    def on_evaluate(self, args, state, control, metrics=None, **kwargs):
        ...
        if combined > self.best_score:
            ...
            print(
                f"✓ New best combined score: {combined:.4f} - saving to Drive..."
            )
...
def language_balanced_sample(
    ...
    for lang, group in df.groupby("lang"):
        ...
        sampled = (
            ...
            .apply(
                lambda x: x.sample(
                    min(len(x), int(cap_n * len(x) / len(group))),
                    random_state=seed,
                )
            )
...
train_df = language_balanced_sample(
    train_df, en_cap=300_000, ru_cap=300_000, other_per_lang=8_000
)
...
n_clean, n_toxic = (train_df["label"] == 0).sum(), (
    train_df["label"] == 1
).sum()
...
print(
    f"Class weights  clean: {class_weights[0]:.3f}, toxic: {class_weights[1]:.3f}"
)
